/*
  # Agregar x-api-key header a comunicación de órdenes

  1. Changes
    - Lee auth_credentials de la configuración
    - Agrega x-api-key header a la llamada HTTP
    - Usa la API key configurada en external_invoice_api_config
*/
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    -- Extraer API key de auth_credentials
    v_api_key := v_config_record.auth_credentials->>'value';
    ...
    -- Llamar a pending-communication API con x-api-key
    BEGIN
      SELECT net.http_post(
        url := v_config_record.api_url,
