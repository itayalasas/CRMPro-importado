/*
  # Actualizar comunicación de órdenes para productos

  1. Changes
    - Detecta si la orden contiene productos (item_type = 'product')
    - Para productos: usa template 'shop_confirmation' con datos de envío
    - Para servicios: mantiene template 'agenda_confirmation' con datos de cita
    - Llama a pending-communication API en lugar de send-order-communication

  2. Templates
    - shop_confirmation: Para órdenes de productos (delivery)
    - agenda_confirmation: Para órdenes de servicios (appointments)
*/
  ...
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
    ...
    IF v_has_products THEN
      ...
      v_delivery_address := COALESCE(
        v_order_metadata->'customer'->>'calle' || ' ' || 
        v_order_metadata->'customer'->>'numero' || ', ' || 
        v_order_metadata->'customer'->>'barrio',
